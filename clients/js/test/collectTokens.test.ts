...
test('it can collect fees from sharded fee legacy token accounts', async (t) => {
...
test('it can collect fees from sharded fee Token 2022 token accounts', async (t) => {
  const client = createDefaultSolanaClient();
  const payer = await generateKeyPairSignerWithSol(client);
  const treasury = address('243cQR2jrZQHQjReVpmq3DjPbqerJZEw7texPYudrfko');
  const mintAuthority = await generateKeyPairSignerWithSol(client);
  ...
  const treasuryAta = await createAta({
    client,
    payer,
    mint,
    owner: treasury,
